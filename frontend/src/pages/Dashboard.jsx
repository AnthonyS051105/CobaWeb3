...
function Dashboard() {
      ...
      try {
        ...
        if (connected && account) {
          ...
          if (positions && positions.suppliedAssets?.length > 0) {
            const factor = await getUserHealthFactor(
              provider,
              account,
              positions.suppliedAssets[0].symbol
            );
  ...
  const totalSupplied =
    userPositions?.suppliedAssets?.reduce(
      (acc, asset) => acc + parseFloat(asset.amountUSD),
      0
    ) || 0;

  const totalBorrowed =
    userPositions?.borrowedAssets?.reduce(
      (acc, asset) => acc + parseFloat(asset.amountUSD),
      0
    ) || 0;
        ...
        <>
          {connected ? (
            ...
            <div className="card mb-8 bg-gradient-to-r from-primary-100 to-secondary-100 border-l-4 border-primary-600">
              ...
              <p className="mb-4">
                Connect your wallet to view your personal dashboard and manage
                your assets.
              </p>
            </div>
          )}
                        ...
                        <tr key={asset.symbol} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-4 whitespace-nowrap">
                            ...
                            <div className="text-xs text-gray-500">
                              {parseFloat(asset.totalSuppliedTokens).toFixed(2)}{" "}
                              {asset.symbol}
                            </div>
                          </td>
                          <td className="px-4 py-4 whitespace-nowrap">
                            ...
                            <div className="text-xs text-gray-500">
                              {parseFloat(asset.totalBorrowedTokens).toFixed(2)}{" "}
                              {asset.symbol}
                            </div>
                          </td>
                          <td className="px-4 py-4 whitespace-nowrap">
                        ...
                        .map((asset, index) => (
                          <Line
                            ...
                            name={`${asset.split("_")[0]} ${
                              asset.includes("supply") ? "Supply" : "Borrow"
                            }`}
                            stroke={
                              asset.includes("supply") ? "#16a34a" : "#7c3aed"
                            }
                            activeDot={{ r: 8 }}
                          />
                        ))}
